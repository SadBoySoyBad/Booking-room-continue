...
Production backend runs application commit `07e3d8b`; frontend remains on `9b31775`, both from `codex/restore-booking-system` (verified 16 September 2026, Asia/Bangkok). Public readiness, guest booking, browser admin approval, settings persistence and concurrent booking checks passed. The owner completed real Google sign-in and their confirmed account now has the admin role. Calendar consent and live event create/approve/cancel/reapprove/delete checks passed on production; QA data was cleaned up. Microsoft is deferred at the owner's request. The branch has not been merged into `main`; future deployments must use the repaired source. See TEST-REPORT.md for evidence and verification limits.
