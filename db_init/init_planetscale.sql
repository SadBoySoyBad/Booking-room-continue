-- Schema for PlanetScale (no foreign keys). Use this when deploying on PlanetScale.
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
    -- Note: no foreign key constraint due to PlanetScale (Vitess) limitations
);

INSERT IGNORE INTO rooms (name, status) VALUES
('Meeting 1', 'AVAILABLE'),
('Meeting 2', 'AVAILABLE'),
('Meeting 3', 'AVAILABLE'),
('Meeting 4', 'AVAILABLE');
