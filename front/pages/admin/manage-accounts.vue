    ...
    <div class="bg-white p-4 rounded-lg shadow-md mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-y-4 md:gap-x-1 items-center mb-4">
        ...
        <div class="flex space-x-2 flex-shrink-0 col-span-1 md:col-start-3 md:col-span-1 justify-center md:justify-end min-w-0">
          <button @click="showFilter = true" class="bg-gray-200 text-gray-800 px-4 py-2 rounded-full hover:bg-gray-300 transition-colors text-sm font-semibold min-w-0">
          ...
          <button @click="exportRows" class="bg-gray-200 text-gray-800 px-4 py-2 rounded-full hover:bg-gray-300 transition-colors text-sm font-semibold min-w-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 inline-block mr-1 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div v-else>
        <AccountsTable :accounts-data="allAccounts" @updated="fetchAccounts(currentPage)" /> <div v-if="allAccounts.length === 0" class="text-center text-gray-500 mt-4">No accounts found.</div>
      </div>
...
const applyFilter = (query) => { filterQuery.value = query; showFilter.value = false; fetchAccounts(1); };
...
const exportRows = async () => {
  try {
    ...
    do {
      const data = await api(`/users?paginate=true&limit=100&page=${page}&role=${activeRoleTab.value}&q=${encodeURIComponent(filterQuery.value)}`);
      rows.push(...data.users); total = data.totalPages; page++;
    } while (page <= total);
...
const fetchAccounts = async (page = 1) => {
  ...
  try {
    let url = `/users?page=${page}&paginate=true&q=${encodeURIComponent(filterQuery.value)}`; // Endpoint สำหรับดึง users ทั้งหมด
    ...
    if (response) {
      ...
      allAccounts.value = response.users.map(user => ({ // Map data from Backend to fit AccountsTable props
        id: user.id,
        email: user.email,
